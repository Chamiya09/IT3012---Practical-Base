...
class VisualGridHuntGame:
    ...
    def execute_action(self, action: str):
        ...
        # Move forward according to facing direction
            ...
            if outside_grid or (next_x, next_y) in self.walls:
                # Wall penalty
                self.score -= 5
            else:
        ...
        if moved and current_pos in self.toxic_traps:
            self.score -= 15
...
class ModelBasedAgent:
    ...
    def position_in_direction(self, position, direction):
        x, y = position
        if direction == "Up":
            return (x, y + 1)
        elif direction == "Down":
    ...
    def update_state(self):
        ...
        # Previous action was MoveForward
            ...
            # If previous percept did NOT detect wall, we assume movement succeeded.
            if self.last_percept is not None and not self.last_percept["wall_ahead"]:
                self.relative_pos = self.position_in_direction(self.relative_pos, self.direction)
                self.visited_cells.add(self.relative_pos)

            # If wall was detected, remember that cell as blocked.
            elif self.last_percept is not None:
                blocked_position = self.position_in_direction(self.relative_pos, self.direction)
                self.blocked_cells.add(blocked_position)

    # Select action using percept + memory
    def sense_and_act(self, percept):
        # FIRST: Update internal state using previous action + previous percept
        self.update_state()

        current = self.relative_pos
        forward_position = self.position_in_direction(current, self.direction)
        left_dir = self.left_direction()
        left_position = self.position_in_direction(current, left_dir)
        right_dir = self.right_direction()
        right_position = self.position_in_direction(current, right_dir)

        # SENSOR MODEL
        # Store current percept
        if percept["wall_ahead"]:
            self.blocked_cells.add(forward_position)
        ...
        # IF wall ahead AND left already visited OR blocked
        # THEN turn right
        elif (
            percept["wall_ahead"]
            and (left_position in self.visited_cells or left_position in self.blocked_cells)
        ):
            action = "TurnRight"
        ...
        # THEN move forward
            ...
            forward_position not in self.visited_cells
            and forward_position not in self.blocked_cells
        ):
            action = "MoveForward"
        ...
        # THEN turn left
            ...
            left_position not in self.visited_cells
            and left_position not in self.blocked_cells
        ):
            action = "TurnLeft"
        ...
        # THEN turn right
            ...
            right_position not in self.visited_cells
            and right_position not in self.blocked_cells
        ):
            action = "TurnRight"

        # Otherwise choose another direction to escape a dead-end
        else:
